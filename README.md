...
GitHub Actions の cron は**指定どおりには起動しない**。
実測（2026-09-07〜12）では `*/5`（5分ごと）と書いたのに、5日間で約35回＝
**2〜4時間に1回**しか起動しなかった（本来なら1,440回）。

そこで防災用は「起動回数」に頼らず、**1回の起動の中でループする**方式にしてある。

| 環境変数 | 既定 | 意味 |
|---|---|---|
| `WATCH_MINUTES` | 170 | 1回の起動で何分間監視し続けるか |
| `WATCH_INTERVAL` | 300 | 何秒ごとにチェックするか（300=5分） |

`python3 tenki_line.py watch` で手元でも試せる。
`cancel-in-progress: true` にしてあるので、次の起動が来たら古い監視を打ち切って新しい監視が始まる。

**Privateリポジトリに戻す場合は `bousai.yml` を必ず無効化すること**（無料枠2,000分を即使い切る）。
Publicなら実行時間は無制限で無料。

### state ファイルの分離

天気用と防災用で別ファイルにして、2つのワークフローが同じファイルを書いてgitで衝突するのを防いでいる。

| ワークフロー | `STATE_FILE` |
|---|---|
| `tenki.yml` | `state.json` |
| `bousai.yml` | `state_bousai.json` |
