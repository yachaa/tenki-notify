...
**30分ごと**にチェックし、送るものがある時だけ届く。

| 種類 | タイミング | 内容 |
|---|---|---|
| 朝のサマリー | 毎朝7時（`config.json`で変更可） | 下記のとおり。1日1通を保証 |
| 雨アラート | 降り出しが近い時だけ | 降り出し時刻・強さ・ピーク・やむ時刻・合計雨量 |
| 警報・注意報 | 発表/解除された時だけ | 新規発表・解除・現在発表中＋雨の実況＋キキクルへの導線 |
| **予報の急変** | 今日の予報が大きく変わった時だけ | 気温・降水確率・雷雨/強風の追加・雨量の増加 |
...
━━ 今日 ━━          天気・最高最低気温（前日比）・体感・湿度
                     降水確率・雨量・雨の継続時間
                     風向と風速（最大瞬間）・日の出/日の入り
━━ 時間帯 ━━        朝/昼/夜それぞれの天気・気温・降水確率
━━ 雨の見込み ━━    降る時間帯と強さ（弱い雨〜非常に激しい雨）
━━ この先1週間 ━━   7日分の天気・気温・降水確率
━━ ひとこと ━━      傘の要否・服装の目安・強風/UV/PM2.5の注意
```

雨アラートは15分刻みの降水予測を使い、同じ降り出しについては1回しか送らない。
夜22時〜朝6時は送らない（`quiet_hours_jst`で変更可）。
